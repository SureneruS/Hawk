...
public class EcoFeature {
	...
	private static List<Transform> generateTransforms() {
		...
		int numberOfTransforms = 
				Helper.getRandomInRange(GAControls.MinimumTransforms, 
				GAControls.MaximumTransforms, 
				randomGenerator);
		
		for(int i = 0; i < numberOfTransforms; i++) {
			try {
				
				boolean isFinalTransform = addTransform(transformList, randomGenerator);
				if(isFinalTransform) {
					break;
				}
			}
	...
	private static boolean addTransform(List<Transform> transformList,
		...
		transformList.add(new ErodeTransform());
		return true;*/
		int randomTransform = Helper.getRandomInRange(0, TransID.values().length - 1, randomGenerator);
		//System.out.println("randT: " + randomTransform + " len: " + TransID.values().length);
		...
		while(!ok) {
			try {
				...
				int startY = Helper.getRandomInRange(0, 
						...
						randomGenerator);
				int width = Helper.getRandomInRange(GAControls.MinimumRegionWidth, 
						GAControls.TrainingImageWidth - startX,
						randomGenerator);
				int height = Helper.getRandomInRange(GAControls.MinimumRegionHeight,
